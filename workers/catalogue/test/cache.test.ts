...
import { GAME_COLUMNS, POPULAR_ORDER, searchConditions, toGame } from "../src/catalogue";
...
describe("production schema", () => {
  ...
  it("requires every selected value from the same filter category", () => {
  ...
  it("selects only real schema columns and keeps every ranking tie-breaker", () => {
    expect(GAME_COLUMNS).not.toContain("game_json");
    expect(POPULAR_ORDER).toContain("steam_positive_ratio * steam_positive_ratio");
    expect(POPULAR_ORDER).toContain("steam_review_count DESC");
    expect(POPULAR_ORDER).toContain("popularity_rank ASC");
    expect(POPULAR_ORDER).toContain("name COLLATE NOCASE ASC");
    expect(POPULAR_ORDER).toContain("CAST(app_id AS INTEGER) ASC");
  });
});
